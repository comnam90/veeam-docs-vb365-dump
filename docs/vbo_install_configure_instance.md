---
title: "Step 8. Configure PostgreSQL Instance"
product: "vb365"
doc_type: "guide"
...
| --- |
| Note |
| Consider considerations and limitations in [Before You Begin](vbo_install_before_you_begin.md). |

To specify settings for the PostgreSQL instance, do the following:

1. Select one of the following options:

* Install new instance. Select this option to install a new PostgreSQL instance. The installation wizard will deploy PostgreSQL 16.14 on the target machine and create a database with the VeeamBackup365 name.
...
If a configuration database with the specified name already exists (for example, it was created by a previous installation of Veeam Backup for Microsoft 365), the installation wizard will notify about it. To connect to the detected database, click Yes. If necessary, Veeam Backup for Microsoft 365 will automatically upgrade the database to the latest version.

![Step 8. Configure PostgreSQL Instance](images/vbo_iso_setup_postgresql_instance.webp "Installing Veeam Backup for Microsoft 365")
