...
| NATS | Local or remote installation of the NATS server 2.10.18, 2.10.22, 2.10.28, 2.12.5.  Note: If you plan to use the NATS server other than the version included in the [Veeam Backup for Microsoft 365 setup](download_package.md), you must install and maintain it yourself. |
...
| Other | The following components are required:   * Microsoft ASP.NET Core Runtime 8.0.10–8.0.xx. * Microsoft .NET Runtime 8.0.10–8.0.xx. For Linux, you must install .NET packages from Microsoft package feed. For more information, see [this Microsoft article](https://learn.microsoft.com/en-us/dotnet/core/install/linux-ubuntu).  * To use PowerShell, Windows PowerShell 7.4.2 or later is required.   For a machine used as a workgroup backup proxy, the following settings are required:   * The Remote Registry service must run on the target machine. The service startup type must be set to Automatic. * [Backup proxy server ports](vbo_used_ports.md#bp_ports) must be opened in a firewall. |
