...
| MailboxFolder.ReadWrite.All | Application | Restore | Accessing Exchange mailbox folders to do the following:   * Read properties and relationships of a mailbox folder.  * Get mailbox folder objects that were added, deleted, or removed from a user mailbox. * Create a new mailbox folder or subfolder in a user mailbox. | new |
